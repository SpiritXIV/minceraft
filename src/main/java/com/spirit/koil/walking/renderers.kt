...
import com.spirit.koil.api.bukkit.*
import com.spirit.koil.api.bukkit.entity.BlockDisplay
import com.spirit.koil.api.bukkit.entity.Display
import com.spirit.koil.api.bukkit.util.Transformation
import com.spirit.koil.api.bukkit.util.Vector
import com.spirit.koil.walking.ChainSegment
import org.joml.*
...
object BlockDisplayRenderer : Renderer {
    ...
    fun renderLeg(leg: Leg, debug: RenderDebugOptions) {
        var root = leg.parent.location.world?.let { leg.chain.root.toLocation(it) }

        val upVector = run {
            // up vector is the cross product of the y-axis and the end-effector direction
            val direction = root?.let { leg.chain.segments.last().position.clone().subtract(it.toVector()) }
        ...
        for ((i, segment) in leg.chain.segments.withIndex()) {
            ...
            root = leg.parent.location.world?.let { segment.position.toLocation(it) }
        }
        ...
        if (leg.didStep) {
            val location = leg.parent.location.world?.let { leg.endEffector.toLocation(it) }
            val volume = .3f
    ...
    fun renderLegTargetPosition(leg: Leg, renderDebug: Boolean) {
        val location = leg.parent.location.world?.let { leg.targetPosition.toLocation(it) }
    ...
    fun renderLegRestPosition(leg: Leg, renderDebug: Boolean) {
        val location = leg.parent.location.world?.let { leg.restPosition.toLocation(it) }
    ...
    fun renderLegEndEffector(leg: Leg, renderDebug: Boolean) {
        val position = leg.parent.location.world?.let { leg.endEffector.toLocation(it) }
...
object ParticleRenderer : Renderer {
    ...
    fun renderLeg(leg: Leg) {
        ...
        var current = world?.let { leg.chain.root.toLocation(it) }
        ...
        for ((i, segment) in leg.chain.segments.withIndex()) {
            ...
            current = world?.let { segment.position.toLocation(it) }
        }
    }
