...
import com.spirit.koil.api.bukkit.FluidCollisionMode
...
import com.spirit.koil.walking.KinematicChain
import kotlin.math.*
...
class Leg(
    ...
    private fun updateMovement() {
        ...
        if (!onGround) {
            ...
            val yCollision = parent.location.world?.let { endEffector.toLocation(it) }
                ?.let { resolveGroundCollision(it) }
            if (yCollision != null) {
    ...
    private fun onGround(): Boolean? {
        return parent.location.world?.let { endEffector.toLocation(it) }?.let { isOnGround(it) }
    }
    ...
    private fun locateGround(): Location? {
        val location = parent.location.world?.let { restPosition.toLocation(it) }
        ...
        // return closest to preferred location
         return candidates.minByOrNull { preferredLocation?.let { it1 -> it?.distanceSquared(it1) } ?: Double.MAX_VALUE }
    }
}
