package com.spirit.tdbtd.global.world.feature;
        ...
        for(int i = 0; i <= length; ++i) {
            if (world.isAir(pos)) {
                if (i == length || !world.isAir(pos.down())) {
                    world.setBlockState(pos, (BlockState) TDBTDBlocks.CRITERIC_VINES_BODY.getDefaultState().with(AbstractPlantStemBlock.AGE, MathHelper.nextInt(random, minAge, maxAge)), Block.NOTIFY_LISTENERS);
                    break;
                }

                world.setBlockState(pos, TDBTDBlocks.CRITERIC_VINES_HEAD.getDefaultState(), Block.NOTIFY_LISTENERS);
            }
