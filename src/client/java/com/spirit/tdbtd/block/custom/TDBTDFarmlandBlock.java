...
public class TDBTDFarmlandBlock
        extends Block {
    public static final IntProperty MOISTURE = Properties.MOISTURE;
    protected static final VoxelShape SHAPE = Block.createCuboidShape(0.0, 0.0, 0.0, 16.0, 15.0, 16.0);
    public static final int MAX_MOISTURE = 7;

    public TDBTDFarmlandBlock(Settings settings) {
        super(settings);
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!state.canPlaceAt(world, pos)) {
            TDBTDFarmlandBlock.setToDirt(state, world, pos);
        }
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (TDBTDFarmlandBlock.isWaterNearby(world, pos) || world.hasRain(pos.up())) {
            if (i < 7) {
        ...
        } else if (!TDBTDFarmlandBlock.hasCrop(world, pos)) {
            TDBTDFarmlandBlock.setToDirt(state, world, pos);
        }
    }
    ...
    public static void setToDirt(BlockState state, World world, BlockPos pos) {
        world.setBlockState(pos, TDBTDFarmlandBlock.pushEntitiesUpBeforeBlockChange(state, TDBTDBlocks.DIMENTED_DIRT.getDefaultState(), world, pos));
    }
